...
function mostrarDetalle(detData){
    var arr_Grid = new Array();
    //console.log(detData);
    if (!sessionStorage.dts_detalleData) {//Entra solo si no existe
        for (let i = 0; i < detData.length; i++) {
            rowGrid = new Object();
            rowGrid.CodigoItem =detData[i]['item_id'];
            rowGrid.DetalleItem =detData[i]['Nombre'];
            rowGrid.CantidadItem =detData[i]['com_cantidad'];
            rowGrid.StockItem =0;//detData[i]['item_id'];
            rowGrid.PrecioItem =detData[i]['com_precio'];
            rowGrid.TotalItem =parseFloat(detData[i]['com_cantidad'])*parseFloat(detData[i]['com_precio']);
            rowGrid.TotalItem=redondea(rowGrid.TotalItem,N2decimal);
            rowGrid.cargaIva =detData[i]['com_graba_iva'];
            arr_Grid[i]=rowGrid;            
        }
...
function guardarOrden() {
    ...
    if ($('#txtCodproveedor').val() != "" && total > 0) {      
            ...
            let link = base_url + '/Compra/ingresarCompra';
            $.ajax({
                type: 'POST',
                url: link,
                data: {
                ...
                success: function (data) {
                    if (data.status) {
                        ...
                        window.location = base_url+'/Compra/compra'; 
...
tableOrdenes = $('#tableOrdenes').dataTable( {
    ...
    "ajax":{
        "url": " "+base_url+"/Compra/getordenCompras",
        "dataSrc":""
    },
    "columns":[
        {"data":"Ncompra"},        
...
//ok
function mostrarDocumento(codigo){
    eliminarStores();
    window.location = base_url+'/Compra/mostrarCompra/'+codigo;
}

function mostrarDocumentoOrden(codigo){
    eliminarStores();
    window.location = base_url+'/Compra/mostrarOrden/'+codigo;
}




function fntAnularCompra(ids){
    swal({
        title: "Anular",
        text: "¿Realmente quiere Anular Compra?",
        type: "warning",
        ...
        if (isConfirm) {
            ...
            let ajaxUrl = base_url+'/Compra/anularCompra';
            let strData = "ids="+ids;
...
window.addEventListener('load', function() {
    //if(accionData=="factura" || accionData=="orden" ){
    if(accionData=="factura"){
        //eliminarStores();
        mostrarDetalle(detData);
    }   
    recargarGridDetalle();
}, false);