...
document.addEventListener('DOMContentLoaded', function(){
	tableMoneda = $('#tableMoneda').dataTable( {
		"aProcessing":true,
		"aServerSide":true,
        "language": {
            "url": cdnTable       
        ...
        "ajax":{
            "url": " "+base_url+"/Moneda/getMonedas",
            "dataSrc":""
        },
        "columns":[
            {"data":"Ids"},
    ...
    //NUEVO
    var formMoneda = document.querySelector("#formMoneda");//Nombre del formulario 
    formMoneda.onsubmit = function(e) {//Se ejecuta en el Summit
        e.preventDefault();//Parar el envio de datos y que se resfresque la pagina
        //Captura de Campos
        var Ids = document.querySelector('#txth_ids').value;
        var mon_simbolo = document.querySelector('#txt_mon_simbolo').value;
        var mon_nombre = document.querySelector('#txt_mon_nombre').value;
        var mon_unidad_cambio = document.querySelector('#txt_mon_unidad_cambio').value;
        var estado = document.querySelector('#cmb_estado').value;       
        if(mon_simbolo == '' || mon_nombre == '' || mon_unidad_cambio == '' || estado == '')//Validacin de Campos
          {
        ...
        var ajaxUrl = base_url+'/Moneda/setMoneda'; 
        var formData = new FormData(formMoneda);//Objeto de Formulario capturado
        ...
        request.onreadystatechange = function(){
           if(request.readyState == 4 && request.status == 200){//Responde  
                ...
                if(objData.status) {
                    $('#modalFormMoneda').modal("hide");//Oculta el Modal
                    formMoneda.reset();//Limpiar los campos del formulario
                    swal("Monedas", objData.msg ,"success");
                    tableMoneda.api().ajax.reload(function(){//Actualizar o refrescar el Datatable de ROL 
...
function fntViewMoneda(ids){
    var ids = ids;
    var request = (window.XMLHttpRequest) ? new XMLHttpRequest() : new ActiveXObject('Microsoft.XMLHTTP');
    var ajaxUrl = base_url+'/Moneda/getMoneda/'+ids;
        ...
        if(request.readyState == 4 && request.status == 200){
            ...
            if(objData.status){
               var estadoReg = objData.data.Estado == 1 ? 
                ...
                document.querySelector("#lbl_sim_moneda").innerHTML = objData.data.Simbolo;
                document.querySelector("#lbl_nom_moneda").innerHTML = objData.data.Nombre;
                document.querySelector("#lbl_unicam_moneda").innerHTML = objData.data.UnidadCambio;
...
//Editar Registro
function fntEditMoneda(ids){
    document.querySelector('#titleModal').innerHTML ="Actualizar Moneda";
    ...
    request.onreadystatechange = function(){
        if(request.readyState == 4 && request.status == 200){
            ...
            if(objData.status){
                ...
                document.querySelector("#txt_mon_simbolo").value = objData.data.Simbolo;
                document.querySelector("#txt_mon_nombre").value = objData.data.Nombre;
                document.querySelector("#txt_mon_unidad_cambio").value = objData.data.UnidadCambio;
...
function fntDeleteMoneda(ids){
    ...
    swal({
        ...
        if (isConfirm) {
            var request = (window.XMLHttpRequest) ? new XMLHttpRequest() : new ActiveXObject('Microsoft.XMLHTTP');
            var ajaxUrl = base_url+'/Moneda/delMoneda';
            var strData = "ids="+ids;
            request.open("POST",ajaxUrl,true);
            ...
            request.onreadystatechange = function(){
                if(request.readyState == 4 && request.status == 200){
                    ...
                    if(objData.status){
                        ...
                        tableMoneda.api().ajax.reload(function(){
