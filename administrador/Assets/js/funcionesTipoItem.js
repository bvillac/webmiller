...
document.addEventListener('DOMContentLoaded', function(){
	tableTipoItem = $('#tableTipoItem').dataTable( {
		"aProcessing":true,
		"aServerSide":true,
        "language": {
            "url": cdnTable       
        ...
        "ajax":{
            "url": " "+base_url+"/TipoItem/getTipoItems",
            "dataSrc":""
        },
        "columns":[
            {"data":"Ids"},
    ...
    //NUEVO
    var formTipoItem = document.querySelector("#formTipoItem");//Nombre del formulario 
    formTipoItem.onsubmit = function(e) {//Se ejecuta en el Summit
        e.preventDefault();//Parar el envio de datos y que se resfresque la pagina
        //Captura de Campos
        var Ids = document.querySelector('#txth_ids').value;
        var tip_nombre = document.querySelector('#txt_tip_nombre').value;
        var estado = document.querySelector('#cmb_estado').value;       
        if(tip_nombre == '' || estado == '')//Validacin de Campos
          {
        ...
        var ajaxUrl = base_url+'/TipoItem/setTipoItem'; 
        var formData = new FormData(formTipoItem);//Objeto de Formulario capturado
        ...
        request.onreadystatechange = function(){
           if(request.readyState == 4 && request.status == 200){//Responde  
                ...
                if(objData.status) {
                    $('#modalFormTipoItem').modal("hide");//Oculta el Modal
                    formTipoItem.reset();//Limpiar los campos del formulario
                    swal("TipoItems", objData.msg ,"success");
                    tableTipoItem.api().ajax.reload(function(){//Actualizar o refrescar el Datatable de ROL 
...
function openModal(){
    ...
    document.querySelector('#titleModal').innerHTML = "Nuevo Tipo de Item";
    document.querySelector("#formTipoItem").reset();
	$('#modalFormTipoItem').modal('show');
}



//FUNCION PARA VISTA DE REGISTRO
function fntViewTipoItem(ids){
    var ids = ids;
    var request = (window.XMLHttpRequest) ? new XMLHttpRequest() : new ActiveXObject('Microsoft.XMLHTTP');
    var ajaxUrl = base_url+'/TipoItem/getTipoItem/'+ids;
...
//Editar Registro
function fntEditTipoItem(ids){
    document.querySelector('#titleModal').innerHTML ="Actualizar Tipo de Item";
    ...
    request.onreadystatechange = function(){
        if(request.readyState == 4 && request.status == 200){
            ...
            if(objData.status){
                ...
                document.querySelector("#txt_tip_nombre").value = objData.data.Nombre;
...
function fntDeleteTipoItem(ids){
    var ids = ids;
    swal({
        ...
        if (isConfirm) {
            var request = (window.XMLHttpRequest) ? new XMLHttpRequest() : new ActiveXObject('Microsoft.XMLHTTP');
            var ajaxUrl = base_url+'/TipoItem/delTipoItem';
            var strData = "ids="+ids;
            request.open("POST",ajaxUrl,true);
            ...
            request.onreadystatechange = function(){
                if(request.readyState == 4 && request.status == 200){
                    ...
                    if(objData.status){
                        ...
                        tableTipoItem.api().ajax.reload(function(){
